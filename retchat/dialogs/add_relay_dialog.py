"""Dialog to add an RRC (Reticulum Relay Chat) hub or join a room."""
...
class AddRelayDialog(_BaseDialog):
    def __init__(self, parent_window: Gtk.Window, service: ReticulumService, on_hub_added: Callable[[str, str], None]):
        ...
        self.set_title("Relay-Chat beitreten")
        self.set_size_request(280, 360)

        page = Adw.PreferencesPage()

        # 1. Quick Join Community Hub
        group_quick = Adw.PreferencesGroup()
        group_quick.set_title("Bekannte Relay-Knoten")
        group_quick.set_description(
            "Öffentliche Community-Hubs zum direkten Loschatten im Reticulum-Mesh."
        )

        quick_row = Adw.ActionRow(
            title=COMMUNITY_HUB_NAME,
            subtitle=f"{COMMUNITY_HUB_HASH[:16]}... • #general"
        )
        quick_btn = Gtk.Button(label="Beitreten", valign=Gtk.Align.CENTER)
        ...
        group_custom.set_title("Eigenen Hub hinzufügen")
        ...
        self.room_row = Adw.EntryRow(title="Chatraum")
        self.room_row.set_text("#general")
        group_custom.add(self.room_row)

        save_row = Adw.ActionRow(title="Hub speichern und beitreten")
    ...
    def _on_quick_join_clicked(self, _btn):
        ...
        if success:
            self.on_hub_added(COMMUNITY_HUB_HASH, "#general")
            self.close()
        else:
    ...
    def _on_custom_join_clicked(self, _btn):
        ...
        room = self.room_row.get_text().strip() or "#general"

        if not hub_hex:
            self._show_toast("Bitte einen Hub-Hash eingeben")
            return

        success, msg = self.service.add_rrc_hub(hub_hex, name=name, initial_room=room)
        if success:
